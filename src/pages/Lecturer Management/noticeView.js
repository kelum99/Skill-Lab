...
function NoticeView() {
  ...
  const fetchNotice = async () => {
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        setNoticeList(result.data.map(vl => ({...vl, date: moment(vl.date).local().format("YYYY-MM-DD")})));
      }
  ...
  return (
    <div className="allT">
      <Search placeholder="Search Notice" onSearch={onSearch} allowClear enterButton className="searchQ" />
